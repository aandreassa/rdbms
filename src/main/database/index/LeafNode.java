...
class LeafNode extends BPlusNode {
  ...
  public Optional<Pair<DataBox, Integer>> put(DataBox key, RecordId rid)
    ...
    if (order * 2 < keys.size()) {                                  // If an overflow will occur
      LeafNode overflowLeaf = new LeafNode(metadata, keys.subList(order, keys.size()), rids.subList(order, keys.size()), rightSibling);   // Create new right sibling leaf
      int overflowPageNum = overflowLeaf.getPage().getPageNum();
      rightSibling = Optional.of(overflowPageNum);                 // Update left leaf's right sibling
      DataBox splitKey = keys.get(order);                          // Find splitKey
      keys = keys.subList(0, order);
      rids = rids.subList(0, order);
      sync();
      return Optional.of(new Pair(splitKey, overflowPageNum));
    }
    sync();
  ...
  public static LeafNode fromBytes(BPlusTreeMetadata metadata, int pageNum) {
    ...
    int rightNeighbor = buf.getInt(); // Get right sibling integer
    ...
    int n = buf.getInt();                                           // Number of key-rids pairs
    for (int i = 0; i < n; ++i) {                                   // Read keys

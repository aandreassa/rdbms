...
public class BPlusTree {
    ...
    public BPlusTree(String filename) {
        ...
        boolean wipe = false;                                                   // False in order to read a file
        PageAllocator allocator = new PageAllocator(filename, wipe);
        ...
        //Page rootPage = allocator.fetchPage(rootPageNum);
        // Maybe check order sizes (?), so they match

        this.metadata = new BPlusTreeMetadata(allocator, keySchema, order);
        this.headerPage = headerPage;
        this.root = BPlusNode.fromBytes(metadata, rootPageNum);
    }
    ...
    public Optional<RecordId> get(DataBox key) {
      typecheck(key);
      LeafNode resultNode = this.root.get(key);
      Optional<RecordId> resultRid = resultNode.getKey(key);
      return resultRid;
      //throw new UnsupportedOperationException("TODO(hw2): implement.");
    }
    ...
    public void put(DataBox key, RecordId rid) throws BPlusTreeException {
      typecheck(key);
      try {
          Optional<Pair<DataBox, Integer>> root = this.root.put(key,rid);
          if (root.isPresent()) {
              DataBox masterKey = root.get().getFirst();
              ArrayList<DataBox> keyList = new ArrayList<>();
              ArrayList<Integer> childrenList = new ArrayList<>();
              keyList.add(masterKey);
              childrenList.add(this.root.getPage().getPageNum());
              childrenList.add(root.get().getSecond());
              this.root = new InnerNode(metadata, keyList, childrenList);
              //this.headerPage = this.root.getPage();
              ByteBuffer headerBuf = headerPage.getByteBuffer();
              //writeHeader(this.root.getPage().getByteBuffer());
              writeHeader(headerBuf);
          }
          //this.root = this.root.put(key, rid);
      } catch (BPlusTreeException b) {
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
      ...
      public BPlusTreeIterator() { // Constructor for scan all
          //startKey = null;
          currLeaf = root.getLeftmostLeaf();
          currIterator = currLeaf.scanAll();
      }

      public BPlusTreeIterator(DataBox key) {
          //startKey = key;
          currLeaf = root.get(key);
          currIterator = currLeaf.scanGreaterEqual(key);
      }
      ...
      public boolean hasNext() {
          ...
          Optional<LeafNode> neighbor = currLeaf.getRightSibling();
          while (neighbor.isPresent()) {
            currLeaf = neighbor.get();
            currIterator = currLeaf.scanAll();
            if (currIterator.hasNext()) {
                return true;
            } else {
                neighbor = currLeaf.getRightSibling();
            }
          }
          return false;
      }

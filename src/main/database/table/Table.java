...
public class Table implements Iterable<Record>, Closeable {
  ...
  public class RIDPageIterator implements BacktrackingIterator<RecordId> {
    ...
    private RecordId markedRecordId = null;
    private RecordId currRecordId = null;
    private RecordId nextRecordId = null;

    public RIDPageIterator(Page page) {
      this.page = page;
      this.bitMap = getBitMap(page);
      this.numRecordsPerPage = getNumRecordsPerPage();

      // Find the first next element
      while (currentIndex < numRecordsPerPage && Bits.getBit(bitMap, currentIndex) != Bits.Bit.ONE) {
        currentIndex += 1;
      }

      if (currentIndex < numRecordsPerPage) {
        //this.nextRecordId = RecordId.fromBytes(page.getByteBuffer());//page.getByteBuffer()
        this.nextRecordId = new RecordId(page.getPageNum(), (short) currentIndex);
        currentIndex++;
      }
    ...
    public RecordId next() {
      if (!hasNext()) {
        ...
        this.currRecordId = nextRecordId;

        while (currentIndex < numRecordsPerPage && Bits.getBit(bitMap, currentIndex) != Bits.Bit.ONE) {
          currentIndex += 1;
        }
        if (currentIndex < numRecordsPerPage) {
          nextRecordId = new RecordId(page.getPageNum(), (short) currentIndex);
          currentIndex++;
        } else {
          nextRecordId = null;
        }
    ...
    public void mark() {
      //throw new UnsupportedOperationException("hw3: TODO");
      if (this.currRecordId == null) {  // Do nothing if current is empty
        return;
      }
      this.markedRecordId = this.currRecordId;
    }

    public void reset() {
      //throw new UnsupportedOperationException("hw3: TODO");
      if (this.markedRecordId == null) {
        return;
      }
      this.currentIndex = this.markedRecordId.getEntryNum() + 1;
      this.nextRecordId = this.markedRecordId;
      this.currRecordId = null;
    }
  }
  ...
  public class RIDBlockIterator implements BacktrackingIterator<RecordId> {
    ...
    public RIDBlockIterator(BacktrackingIterator<Page> block) {
      ...
      if (this.block.hasNext()) {
        ...
        while (!blockIter.hasNext() && block.hasNext()) {
          blockIter = new RIDPageIterator(this.block.next());
        }
    ...
    public boolean hasNext() {
      //throw new UnsupportedOperationException("hw3: TODO");
      //return blockIter != null && blockIter.hasNext();

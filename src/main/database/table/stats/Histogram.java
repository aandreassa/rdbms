...
public class Histogram {
  ...
  public void buildHistogram(Table table, int attribute){
      ...
      for (Record r : table) {
        ...
        float quantizedValue = quantization(r, attribute);
        if (minVal > quantizedValue) {
          minVal = quantizedValue;
        }
        if (maxVal < quantizedValue) {
          maxVal = quantizedValue;
        }
        count++;
      }
      ...
      this.width = (maxValue - minValue)/this.numBuckets;

      //3. create each bucket object
      for (int bucketSlot = 0; bucketSlot < numBuckets - 1; bucketSlot++) {
        Bucket<Float> b = new Bucket<Float>(minValue + width * bucketSlot, minValue + width * (bucketSlot + 1));
        this.buckets[bucketSlot] = b;
      }
      // Handle logic for the last value (max)
      this.buckets[numBuckets - 1] = new Bucket<Float>(minValue + width * (numBuckets - 1), maxVal);

      //4. populate the data using the increment(value) method
      for (Record r : table) {

        float value = quantization(r, attribute);
        // Edge case: width is 0, therefore add one value and set count to number of items.

        if (width == 0) {
          Bucket<Float> b = buckets[numBuckets - 1];
          b.increment(value);
          b.setCount(count);
          break;
        }

        int bucketSlot = getBucketSlot(value);
        this.buckets[bucketSlot].increment(value);
      }
  }

  public int getBucketSlot(float value) {
    int bucketSlot;
    ...
    if (value == maxValue){
      bucketSlot = numBuckets - 1;
    } else {
      bucketSlot = (int) (Math.ceil((value - minValue)/width) - 1);
      if ((value - minValue) % width == 0) {
        bucketSlot += 1;
      }
    }
  ...
  private float [] allEquality(float qvalue){
    ...
    int bucketSlot = getBucketSlot(qvalue);

    // If qvalue is not in the buckets
    if (bucketSlot == -1 || bucketSlot == -2) {
      return result;
    }

    result[bucketSlot] = (float) 1.0/get(bucketSlot).getDistinctCount();
 ...
 /**
  ...
  private float [] allGreaterThan(float qvalue){
    ...
    int bucketSlot = getBucketSlot(qvalue);
    if (bucketSlot == -2) {
      Arrays.fill(result, 0, numBuckets, 1.0f);
      return result;
    }

    if (bucketSlot == -1) {
      Arrays.fill(result, 0, numBuckets, 0.0f);
      return result;
    }

    Bucket<Float> bucket = get(bucketSlot);
    Arrays.fill(result, 0, bucketSlot, 0.0f);
    Arrays.fill(result,bucketSlot + 1, numBuckets, 1.0f);
    result[bucketSlot] = (bucket.getEnd() - qvalue)/width;
    // TODO: HW4 implement;
 ...
 /**
  ...
  private float [] allLessThan(float qvalue){
    ...
    // TODO: HW4 implement;
    int bucketSlot = getBucketSlot(qvalue);
    if (bucketSlot == -2) { // Smaller than min
      Arrays.fill(result, 0, numBuckets, 0.0f);
      return result;
    }

    if (bucketSlot == -1) { // Larger than max
      Arrays.fill(result, 0, numBuckets, 1.0f);
      return result;
    }

    Bucket<Float> bucket = get(bucketSlot);
    Arrays.fill(result, 0, bucketSlot, 1.0f);
    Arrays.fill(result,bucketSlot + 1, numBuckets, 0.0f);
    result[bucketSlot] = (qvalue - bucket.getStart())/width;
    // TODO: HW4 implement;
